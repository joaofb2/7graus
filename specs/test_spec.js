...
describe('Amazon Test', () => {
  ...
  it('O utilizador vai á página da Amazon, depois pesquisa por raton e é confirmado que a pesquisa foi feita', () => {
    browser.get('https://www.amazon.es');
    element(by.id('twotabsearchtextbox')).sendKeys("raton");browser.sleep(3000)
    element(by.id('nav-search-submit-button')).click();
    expect(by.id('B01618WO9O-amazons-choice-label'));
  });

  it('O utilizador clica no primeiro botao pelo text dele', () => {
    browser.sleep(1000)
  ...
  it('O utilizador é direccionado para a página de Login. Onde põe email e password', () => {
  ...
  it('O utilizador chega á 1ª página do Pagamento - Direção ', () => {
    browser.sleep(1000)
